...
# Знаходження найкоротшого шляху між усіма вершинами
for source in stations:
    for target in stations:
        if source != target:
            try:
                shortest_path = nx.dijkstra_path(G, source=source, target=target)
                shortest_distance = nx.dijkstra_path_length(G, source=source, target=target)
                print(f"Найкоротший шлях від '{source}' до '{target}': {shortest_path}")
                print(f"Відстань: {shortest_distance}")
                print("-" * 30)
            except nx.NetworkXNoPath:
                print(f"Шлях від '{source}' до '{target}' не знайдено.")
